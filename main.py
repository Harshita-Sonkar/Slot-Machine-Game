...
def deposit():
    while True:
        amount = input("What would you like to deposit (between Rs.0 and Rs.2000)? Rs.")
        if amount.isdigit():
            amount = int(amount)
            if 2000 >= amount > 0:
                break
            else:
                print("Amount must be greater than 0 and less than 2000.")
        else:
...
def get_bet():
    while True:
        amount = input("How much would you like to bet on each line? Rs.")
        if amount.isdigit():
            amount = int(amount)
            if MIN_BET <= amount <= MAX_BET:
                break
            else:
                print(f"Amount must be greater between Rs.{MIN_BET} - Rs.{MAX_BET}.")
        else:
            print("Please enter a number.")
    return amount
